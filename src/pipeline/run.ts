...
/**
 * Runs the full pipeline. Extracted from main.ts so it can be called
 * from both the CLI entry point and the API trigger endpoint.
 */
export async function runPipeline(db: Db, config: Config): Promise<string> {
  ...
  try {
    ...
    if (googleToken) {
      const count = await syncGoogleContacts(db, googleToken);
